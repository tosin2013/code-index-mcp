...
def create_mock_service(mock_conn):
    ...
    return SemanticSearchService(
        db_connection_string="postgresql://test", embedder=mock_embedder, use_mock=False
    )
...
def test_service_initialization():
    ...
    # Test with real mode
    service = SemanticSearchService(db_connection_string="postgresql://test", use_mock=False)
...
def test_semantic_search_basic(mock_connect):
    ...
    # Perform search
    user_id = uuid4()
    results = service.semantic_search(query="authentication logic", user_id=user_id, top_k=10)
...
def test_semantic_search_min_similarity(mock_connect):
    ...
    user_id = uuid4()
    results = service.semantic_search(
        query="authentication", user_id=user_id, min_similarity=0.90, top_k=10
    )
...
def test_find_similar_code(mock_connect):
    ...
    results = service.find_similar_code(code_snippet=code_snippet, user_id=user_id, top_k=5)
...
def test_search_by_function_name(mock_connect):
    ...
    results = service.search_by_function_name(
        function_name="authenticate", user_id=user_id, fuzzy=True
    )
...
def test_mock_mode():
    ...
    user_id = uuid4()
    results = service.semantic_search(query="test query", user_id=user_id, top_k=10)
