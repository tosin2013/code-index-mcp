...
from code_index_mcp.ingestion import chunk_file, chunk_directory, ChunkStrategy
...
def test_directory():
    ...
    for file_path, chunks in sorted(results.items()):
        ...
        for chunk in chunks:
            print(f"    - {chunk.chunk_type}: {chunk.chunk_name} (lines {chunk.line_start}-{chunk.line_end})")
