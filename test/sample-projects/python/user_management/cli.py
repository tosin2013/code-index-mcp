...
from .utils.exceptions import DuplicateUserError, UserNotFoundError
...
@click.option(
    "--role", type=click.Choice(["user", "admin", "guest"]), default="user", help="Role of the user"
)
@click.option("--password", prompt=True, hide_input=True, help="Password for the user")
@click.pass_context
def create_user(
    ctx, name: str, username: str, age: int, email: Optional[str], role: str, password: str
):
    """Create a new user."""
    try:
