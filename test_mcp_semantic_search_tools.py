...
from code_index_mcp.server import find_similar_code, ingest_code_for_search, semantic_search_code
...
class TestSemanticSearchMCPTools(unittest.TestCase):
    ...
    def test_semantic_search_code_mock_mode(self):
        ...
        result = semantic_search_code(
            ctx=self.ctx, query="authentication with JWT", language="python", top_k=5
        )
    ...
    def test_ingest_code_for_search_no_project_path(self):
        ...
        result = ingest_code_for_search(ctx=self.ctx, use_current_project=True)
    ...
    def test_semantic_search_code_with_alloydb(self, mock_search):
        ...
        result = semantic_search_code(ctx=self.ctx, query="JWT authentication", top_k=5)
    ...
    def test_find_similar_code_with_alloydb(self, mock_find):
        ...
        result = find_similar_code(
            ctx=self.ctx, code_snippet="def check_password(p): return True", top_k=3
        )
    ...
    def test_ingest_code_for_search_with_alloydb(self, mock_ingest):
        ...
        result = ingest_code_for_search(
            ctx=self.ctx, use_current_project=True, project_name="test-project"
        )
