...
from .models import FileAnalysisResult, RelationshipGraph
...
class RelationshipTracker:
        ...
        for result in analysis_results:
            ...
            for func in result.functions:
                ...
                if valid_instantiations:
                    # Store both qualified and unqualified relationships
                    instantiator_qualified = generate_qualified_name(
                        result.file_info.path, func.name
                    )
                    ...
                    for class_name in valid_instantiations:
                        ...
                        for target_file_id in target_files:
                            target_file_path = self.file_id_to_path[target_file_id]
                            qualified_instantiation = generate_qualified_name(
                                target_file_path, class_name
                            )
                            qualified_instantiations.append(qualified_instantiation)

                    self.qualified_class_instantiations[
                        instantiator_qualified
                    ] = qualified_instantiations
    ...
    def _extract_python_function_calls(self, content: str) -> List[str]:
        ...
        for match in re.finditer(pattern, content):
            ...
            if func_name not in [
                'if', 'for', 'while', 'with', 'try', 'except', 'print', 'len',
    ...
    def _extract_javascript_function_calls(self, content: str) -> List[str]:
        ...
        for match in re.finditer(pattern, content):
            ...
            if func_name not in [
                'if', 'for', 'while', 'switch', 'try', 'catch', 'console',
    ...
    def _extract_java_function_calls(self, content: str) -> List[str]:
        ...
        for match in re.finditer(pattern, content):
            ...
            if method_name not in [
                'if', 'for', 'while', 'switch', 'try', 'catch', 'System', 'String'
            ]:
    ...
    def _extract_go_function_calls(self, content: str) -> List[str]:
        ...
        for match in re.finditer(pattern, content):
            ...
            # Skip common keywords
            if func_name not in [
                'if', 'for', 'switch', 'select', 'go', 'defer', 'make', 'new',
                'len', 'cap'
            ]:
                calls.append(func_name)
    ...
    def _extract_c_function_calls(self, content: str) -> List[str]:
        ...
        for match in re.finditer(pattern, content):
            ...
            if func_name not in [
                'if', 'for', 'while', 'switch', 'printf', 'scanf', 'malloc',
                'free', 'sizeof'
            ]:
                calls.append(func_name)
    ...
    def _extract_csharp_function_calls(self, content: str) -> List[str]:
        ...
        for match in re.finditer(pattern, content):
            ...
            # Skip common keywords
            if method_name not in [
                'if', 'for', 'while', 'switch', 'try', 'catch', 'Console', 'String'
            ]:
                calls.append(method_name)
    ...
    def _build_reverse_lookups(self) -> Dict[str, Any]:
        ...
        for caller_qualified, callees_qualified in self.qualified_function_calls.items():
            try:
                ...
                for callee_qualified in callees_qualified:
                    try:
                        _, callee_name = parse_qualified_name(callee_qualified)
                        ...
                        if (
                            unqualified_entry not in
                            reverse_lookups['function_callers'][callee_name]
                        ):
                            reverse_lookups['function_callers'][callee_name].append(
                                unqualified_entry
                            )
        ...
        for (
            instantiator_qualified, classes_qualified
        ) in self.qualified_class_instantiations.items():
            try:
                instantiator_file_path, instantiator_name = parse_qualified_name(
                    instantiator_qualified
                )
                ...
                for class_qualified in classes_qualified:
                    try:
                        _, class_name = parse_qualified_name(class_qualified)
