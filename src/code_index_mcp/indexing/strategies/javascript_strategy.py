...
class JavaScriptParsingStrategy(ParsingStrategy):
    ...
    def parse_file(self, file_path: str, content: str) -> Tuple[Dict[str, SymbolInfo], FileInfo]:
        ...
        self._traverse_js_node(
            tree.root_node, content, file_path, symbols, functions, classes, imports, exports
        )
    ...
    def _traverse_js_node(
        ...
        # Handle arrow functions and function expressions in lexical declarations (const/let)
            ...
            for child in node.children:
                if child.type == "variable_declarator":
                    ...
                    if name_node and value_node:
                        ...
                        signature = (
                            content[child.start_byte : child.end_byte].split("\n")[0].strip()
                        )
                        symbols[symbol_id] = SymbolInfo(
                            type="function",
                            file=file_path,
            ...
            if name:
                symbol_id = self._create_symbol_id(file_path, name)
                symbols[symbol_id] = SymbolInfo(
                    type="class", file=file_path, line=node.start_point[0] + 1
                )
                classes.append(name)
            ...
            if method_name and class_name:
                ...
                signature = self._get_js_function_signature(node, content)
                symbols[symbol_id] = SymbolInfo(
                    type="method", file=file_path, line=node.start_point[0] + 1, signature=signature
                )
        ...
        for child in node.children:
            self._traverse_js_node(
                child, content, file_path, symbols, functions, classes, imports, exports
            )
