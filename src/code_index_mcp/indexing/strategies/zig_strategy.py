...
class ZigParsingStrategy(ParsingStrategy):
    ...
    def _tree_sitter_parse(
        ...
        self._traverse_zig_node(
            tree.root_node, content, file_path, symbols, functions, classes, imports
        )
    ...
    def _traverse_zig_node(
        ...
        if node.type == "function_declaration":
            ...
            if type_name:
                ...
                symbols[symbol_id] = SymbolInfo(
                    type=node.type.replace("_declaration", ""), file=file_path, line=line_number
                )
