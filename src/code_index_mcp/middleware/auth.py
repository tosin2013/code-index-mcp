...
class AuthMiddleware:
    ...
    async def authenticate(self, api_key: str) -> UserContext:
        ...
        if not api_key.startswith("ci_"):
            raise AuthenticationError("Invalid API key format. " "Keys must start with 'ci_'")
    ...
    async def _authenticate_gcp(self, api_key: str) -> UserContext:
        ...
        try:
            ...
            request = secretmanager.ListSecretsRequest(
                parent=parent, filter=f"labels.service:code-index-mcp"
            )
            ...
            for secret in secrets:
                ...
                response = self.secret_client.access_secret_version(name=secret_version)
            ...
            await self._ensure_user_exists(
                user_id=user_id, api_key=api_key, secret_name=valid_secret.name.split("/")[-1]
            )
    ...
    async def _ensure_user_exists(self, user_id: str, api_key: str, secret_name: str) -> None:
        """
