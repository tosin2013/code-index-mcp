...
class VertexAIEmbedder:
    ...
    def _enforce_rate_limit(self):
        ...
        self.config._request_times = [t for t in self.config._request_times if now - t < window]
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                embeddings = self._model.get_embeddings(
                    [text_input], output_dimensionality=self.config.dimensions
                )
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    raise RuntimeError(
                        f"Failed to generate embedding after {self.config.max_retries} attempts: {e}"
                    )

    def generate_embeddings_batch(
        self, texts: List[str], task_type: Optional[str] = None, show_progress: bool = False
    ) -> List[List[float]]:
        """
        ...
        for i in range(0, len(texts), self.config.batch_size):
            ...
            for attempt in range(self.config.max_retries):
                try:
                    ...
                    text_inputs = [
                        self._TextEmbeddingInput(text=text, task_type=task_type) for text in batch
                    ]

                    # Generate embeddings for batch
                    embeddings = self._model.get_embeddings(
                        text_inputs, output_dimensionality=self.config.dimensions
                    )
                    ...
                    if attempt < self.config.max_retries - 1:
                        ...
                        raise RuntimeError(
                            f"Failed to generate batch embeddings after {self.config.max_retries} attempts: {e}"
                        )
    ...
    def embed_code_chunks(
        ...
        embeddings = self.generate_embeddings_batch(
            texts, task_type="RETRIEVAL_DOCUMENT", show_progress=show_progress
        )
...
def generate_embedding(text: str, config: Optional[EmbeddingConfig] = None) -> List[float]:
    """
...
def generate_embeddings_batch(
    texts: List[str], config: Optional[EmbeddingConfig] = None, show_progress: bool = False
) -> List[List[float]]:
    """
...
class MockVertexAIEmbedder:
    ...
    def embed_code_chunks(self, chunks: List[Any], **kwargs) -> List[Tuple[Any, List[float]]]:
