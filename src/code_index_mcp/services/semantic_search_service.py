...
class SemanticSearchService:
    ...
    def semantic_search(
        ...
        query_embedding = self.embedder.generate_embedding(query, task_type="RETRIEVAL_QUERY")
        ...
        try:
            ...
            params = {"user_id": str(user_id), "top_k": top_k}
        ...
        if keyword_filter:
            ...
            for result in results:
                ...
                if (
                    ...
                    or (result.chunk_name and keyword_lower in result.chunk_name.lower())
                    or keyword_lower in result.file_path.lower()
                ):
    ...
    def search_by_function_name(
        ...
        results = self.semantic_search(
            query=query, user_id=user_id, project_name=project_name, top_k=20
        )
        ...
        if fuzzy:
            ...
            # Exact match
            function_results = [
                r for r in function_results if r.chunk_name and r.chunk_name == function_name
            ]
...
def semantic_search(
    ...
    """
    service = SemanticSearchService(db_connection_string=db_connection_string, use_mock=use_mock)

    results = service.semantic_search(
        query=query, user_id=user_id, project_name=project_name, language=language, top_k=top_k
    )
...
def find_similar_code(
    ...
    results = service.find_similar_code(
        code_snippet=code_snippet, user_id=user_id, project_name=project_name, top_k=top_k
    )
