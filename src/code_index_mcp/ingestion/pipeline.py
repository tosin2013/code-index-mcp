...
from ..embeddings.vertex_ai import EmbeddingConfig, MockVertexAIEmbedder, VertexAIEmbedder
from .chunker import ChunkStrategy, CodeChunk, CodeChunker
...
class IngestionPipeline:
    ...
    def _ensure_schema_exists(self, conn):
        ...
        with conn.cursor() as cur:
            ...
            if table_exists:
                ...
                if has_project_id:
                    logger.info(
                        "✓ Database schema already exists with correct structure (normalized with project_id)"
                    )
                    return
                else:
                    logger.warning(
                        "⚠️  code_chunks table exists but has old schema structure. Manual migration required."
                    )
                    raise ValueError(
    ...
    def _get_or_create_project(
        self, conn, user_id: UUID, project_name: str, gcs_bucket: str, gcs_prefix: str
    ) -> UUID:
        """
    ...
    def _update_project_stats(self, conn, project_id: UUID, total_chunks: int, total_files: int):
    ...
    def ingest_directory(
        ...
        try:
                ...
                for file_path in dir_path.rglob(pattern):
                    if file_path.is_file() and str(file_path) not in processed_files:
                        try:
                            ...
                            # Chunk file
                            chunks = self.chunker.chunk_file(str(file_path), content, relative_path)
            ...
            # Generate embeddings
            self._report_progress(
                f"Generating embeddings for {len(all_chunks)} chunks...", stats.to_dict()
            )

            chunks_with_embeddings = self.embedder.embed_code_chunks(
                all_chunks, use_metadata=True, show_progress=True
            )

            stats.embeddings_generated = len(chunks_with_embeddings)

            self._report_progress(
                f"Generated {stats.embeddings_generated} embeddings", stats.to_dict()
            )
            ...
            self._update_project_stats(
                conn, project_id, stats.chunks_inserted, stats.files_processed
            )
            ...
            self._report_progress(f"✅ Ingestion complete!", stats.to_dict())
        ...
        try:
            ...
            for file_path in file_paths:
                try:
                    ...
                    # Chunk file
                    chunks = self.chunker.chunk_file(str(path), content, str(path.name))
