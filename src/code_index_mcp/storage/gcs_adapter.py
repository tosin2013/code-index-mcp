...
from .base_adapter import BaseStorageAdapter, FileMetadata, StorageError, StreamUploadMixin
...
class GCSAdapter(BaseStorageAdapter, StreamUploadMixin):
    ...
    def __init__(
        self, bucket: str, user_id: str, project_name: str, project_id: Optional[str] = None
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"Initialized GCS adapter: " f"bucket={bucket}, namespace={self.namespace_prefix}"
            )
    ...
    async def upload_file(
        ...
        try:
            ...
            return FileMetadata(
                ...
                size=blob.size,
                modified_time=(
                    blob.updated.isoformat() if blob.updated else datetime.utcnow().isoformat()
                ),
    ...
    async def list_files(self, prefix: str = "", recursive: bool = True) -> List[FileMetadata]:
        ...
        try:
            ...
            if recursive:
                ...
                blobs = self.client.list_blobs(self.bucket_name, prefix=full_prefix, delimiter="/")
    ...
    async def download_file_stream(
        self, path: str, chunk_size: int = 1024 * 1024  # 1MB chunks
    ) -> AsyncIterator[bytes]:
        """
